...
class Brain(object):
    ...
    def _build_model(self):
        ...
        if self.dueling:
            ...
            # una serie de capas completamente conectadas para estimar V(s)

            y11 = Dense(self.num_nodes, activation='relu')(x)
            y12 = Dense(self.num_nodes, activation='relu')(y11)
            y13 = Dense(1, activation="linear")(y12)

            # una serie de capas completamente conectadas para estimar A (s, a)
    ...
    def predict(self, state, target=False):
        if target:  # obtiene predicción de la red objetivo
            return self.model_.predict(state)
        else:  # obtiene predicción de la red local
